...
function savePartner(input) { return locked('ACCOUNTANT', app => app.savePartner(input)); }
function createDocument(input) { return locked('ACCOUNTANT', app => app.createDocument(input)); }
function recordPayment(input) { return locked('ACCOUNTANT', app => app.recordPayment(input)); }
function voidRecord(table, id) { return locked('ADMIN', app => app.voidRecord(table, id)); }
function savePromise(input) { return locked('ACCOUNTANT', app => app.savePromise(input)); }
function saveBankAccount(input) { return locked('ADMIN', app => app.saveBankAccount(input)); }
function getVietQr(documentId, accountId) { return locked('VIEWER', app => app.getVietQr(documentId, accountId)); }
function processReminders() { return locked('ACCOUNTANT', app => app.processReminders()); }
...
function commitCsv(entityType, csvText, fileName = 'import.csv') { return locked('ACCOUNTANT', app => prepareImport(app, entityType, csvText, fileName, true)); }
...
function enrichedBootstrap(app) {
  const data = app.bootstrap(); const members = app.store.all('THANH_VIEN').filter(row => row.Trang_Thai === 'HOAT_DONG').map(row => ({ email: row.Email, role: row.Vai_Tro })); const health = calculateHealth(app);
  const workspace = { workspaceId: app.workspace.workspaceId, rootFolderId: app.workspace.rootFolderId, rootFolderUrl: app.workspace.rootFolderUrl, spreadsheetId: app.workspace.spreadsheetId, spreadsheetUrl: app.workspace.spreadsheetUrl, email: app.identity.email, role: app.identity.role }; refreshConsole(app, data, health); return { ...data, workspace, members, health };
}
...
function prepareImport(app, entityType, csvText, fileName, commit) {
  ...
  if (commit && errors.length) throw new Error(`Không import vì còn ${errors.length} dòng lỗi. Hãy sửa và kiểm tra lại.`); let success = 0; if (commit) valid.forEach(item => { if (entityType === 'DOI_TUONG') app.savePartner(item.row); else app.createDocument(item.row); success += 1; });
...
function calculateHealth(app) {
  ...
  const score = Math.max(0, 100 - issues.reduce((sum, issue) => sum + (issue.level === 'HIGH' ? 25 : 10), 0)); return { score, issues, metrics: { partners: partners.length, documents: docs.length, members: members.length, backups: backups.length, lastBackupAt: lastBackup?.updatedAt || '', duplicateNames, invalidEmails, orphanDocuments } };
}
