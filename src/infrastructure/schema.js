...
function protectCalculatedColumns(sheet, headers) { ['Trang_Thai', 'Loi'].forEach(header => { const column = headers.indexOf(header) + 1; if (column < 1) return; const range = sheet.getRange(6, column, 995, 1); if (!range.getProtections(SpreadsheetApp.ProtectionType.RANGE).length) range.protect().setDescription(`FINDEBT: ${header} do hệ thống cập nhật`).setWarningOnly(true); }); }
