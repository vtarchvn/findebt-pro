...
## Kiểm tra tài sản giao diện Apps Script

- Sau `npm run build`, cả `dist/Index.html` và `dist/Client.html` không được còn đường dẫn tương đối `assets/vtarch-symbol-256.png`.
- Logo thanh bên, màn hình khởi động và trang `Tác giả & hỗ trợ` phải dùng cùng data URL đã nhúng để hoạt động trên Google Apps Script.
